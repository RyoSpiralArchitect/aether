...
class StreamingTextDataset(IterableDataset):
    def __init__(
        ...
        self.token_batch_chars = max(0, token_batch_chars)
    ...
    def __iter__(self):
        ...
        def _token_worker():
            buf: List[str] = []
            buf_size = 0
            batch_chars = self.token_batch_chars
            try:
                while True:
                    chunk = raw_q.get()
                    if chunk is stop:
                        raw_q.task_done()
                        if buf:
                            tok_q.put(_encode_text("".join(buf)))
                            buf.clear()
                            buf_size = 0
                        break
                    ...
                    buf.append(chunk)
                    buf_size += len(chunk)
                    flush = batch_chars > 0 and buf_size >= batch_chars
                    if flush:
                        tok_q.put(_encode_text("".join(buf)))
                        buf.clear()
                        buf_size = 0
