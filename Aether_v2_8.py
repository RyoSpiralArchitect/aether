...
class MPSTurboGovernor:
    ...
    def _shed_metrics(self, step: int, ema: float, low_bar: float):
        ...
        tr._turbo_toggle_aux(True, reason=f"ema={ema:.1f}<target")
...
class AetherTrainerBase:
    ...
    def _turbo_toggle_aux(self, disable: bool, reason: Optional[str] = None):
        disable = bool(disable)
        reason_note = ""
        if reason:
            reason_note = f" ({reason})"
        if disable and not getattr(self, "_turbo_disable_aux", False):
            print(f"[TURBO] auxiliary controllers paused{reason_note}")
        elif (not disable) and getattr(self, "_turbo_disable_aux", False):
            print("[TURBO] auxiliary controllers resumed")
        self._turbo_disable_aux = disable
    ...
    def _compute_logits(self, ids: torch.Tensor):
        if getattr(self, "_turbo_disable_aux", False):
            logits = self.model(ids)
            zero = torch.zeros((), device=ids.device, dtype=torch.float32)
            return logits, {"mv_reg": zero, "two_view": zero}
                    ...
                    while idx < B:
                        ...
                        try:
                            with torch.autocast(
                                ...
                                _pf = {}
                                if not getattr(self, "_turbo_disable_aux", False):
                                    try:
                            ...
                            with torch.autocast(device_type="mps", enabled=False):
                                ...
                                if (
                                    ...
                                    and not getattr(self, "_turbo_disable_aux", False)
                                ):
                                    try:
                                        with torch.no_grad():
                                            probs = torch.softmax(
                                ...
                                if (
                                    getattr(self.cfg, "intent_weight", 0.0) > 0.0
                                    and not getattr(self, "_turbo_disable_aux", False)
                                ):
